%% R-component data reflection imaging
...
[data_R_raw,header,~] = ReadSu('/data/Chenglong/Yellowstone_vibroseis/su_data_sta_lta/combine.R.su');

%----remove bad stations which record only one trace----
header([8354,8011,8006,8181,8178]) = [];
data_R_raw(:,[8354,8011,8006,8181,8178]) = [];
%-------------------------------------------------------
...
data_R = data_R_raw(:,order_offset);


figure;imagesc(data_R);colormap(colorbar_bwr);colorbar;clim([0.5,2.5]);
...
%muteD_ipt1=[1,110]; muteD_ipt2=[1667,270]; muteD_ipt3=[11218,900]; %cut direct
muteD_ipt1=[1,110]; muteD_ipt2=[1667,480]; muteD_ipt3=[11218,900]; %cut direct+pp reflection (test 1)
%muteD_ipt1=[1,110]; muteD_ipt2=[724,438]; muteD_ipt3=[11218,900]; %cut direct+pp reflection (test 2)
...
imagesc(10:100:10000,1:2001,R_stack);colormap(colorbar_bwr);clim([0.5,1.5]);%2.5
...
hold on;plot([geom_tab(muteA_ipt1(1),5),geom_tab(muteA_ipt2(1),5)],[muteA_ipt1(2),muteA_ipt2(2)],'k-');title('mute: stacked R');
subplot(1,2,2);
imagesc(data_R);colormap(colorbar_bwr);clim([0.5,clip_value]);
...
hold on;plot([muteA_ipt1(1),muteA_ipt2(1)],[muteA_ipt1(2),muteA_ipt2(2)],'k-');title('mute: raw R');
%**************************************************************************
...
for i=1:length(it_muteD)
    data_R_muteD(1:it_muteD(i),muteD_ipt1(1)-1+i)=value4muted;
end
it_muteA = floor(muteA_ipt1(2)+(muteA_ipt2(2)-muteA_ipt1(2))*((muteA_ipt1(1):muteA_ipt2(1))-muteA_ipt1(1))/(muteA_ipt2(1)-muteA_ipt1(1)));
data_R_muteA = data_R_muteD;
for i=1:length(it_muteA)
    data_R_muteA(it_muteA(i):nt,muteA_ipt1(1)-1+i)=value4muted;
end
if ~isempty(ioffset_range)
    data_R_muteA(:,setdiff(1:ntr,ioffset_range))=value4muted;
end
if ~isempty(clip_value)
    data_R_muteA(data_R_muteA>clip_value)=clip_value;
end

figure;imagesc(data_R_muteA);colormap(colorbar_bwr);colorbar;title('final input data for imaging');
...
dat_dir = '/home0/cxd170430/codes/ACOUSTIC2D_RTM/yellowstone_field_data/data/stalta_r_ps_testmute1/';
tshift = 0.1;

%cell_rec_dat = extract_shot_gathers(data_R_muteA, geom_tab, src_filename, rec_dir, dat_dir, tshift, dt);
cell_rec_dat = extract_shot_gathers(data_R_muteA, geom_tab, '', '', dat_dir, tshift, dt);


%% PP PS imaging

data_mat = data_R_muteA;
nx = 81; %121  81   61
dx = 375; %250  375  500
nz = 201;
dz = 50;
...
% (optional)
if snr_qc==0
    [A_all_R, A_each_R, data_mat_sg] = PP_imaging_srcloop(data_mat, geom_tab, vp, nx, dx, nz, dz, dt, tshift);
else
    data_mat_qc = SNR(su_file_env, data_mat, 'Z', snr_threshold);
    [A_all_R, A_each_R, data_mat_sg] = PP_imaging_srcloop(data_mat_qc, geom_tab, vp, nx, dx, nz, dz, dt, tshift);
end

fp=fopen('/home0/cxd170430/codes/matlab/yellowstone_imaging/output/img375_pp_r_nosig.bin','wb');
fwrite(fp,A_all_R,'float64');fclose(fp);


% (required)
if snr_qc==0
    [A1_all_R, A1_each_R, ccp_x_section] = PS_imaging_srcloop(data_mat, geom_tab, vp, vs, nx, dx, nz, dz, dt, tshift);
else
    data_mat_qc = SNR(su_file_env, data_mat, 'Z', snr_threshold);
    [A1_all_R, A1_each_R, ccp_x_section] = PS_imaging_srcloop(data_mat_qc, geom_tab, vp, vs, nx, dx, nz, dz, dt, tshift);
end

fp=fopen('/home0/cxd170430/codes/matlab/yellowstone_imaging/output/img375_ps_r.bin','wb');
fwrite(fp,A1_all_R,'float64');fclose(fp);
...
figure;imagesc(axis_x,axis_z,A_all_R+A1_all_R);colormap jet;colorbar;title('PP + PS');xlabel('Distance (km)');ylabel('Depth (km)');
figure;imagesc(axis_x,axis_z,A_all_R.*A1_all_R);colormap jet;colorbar;title('PP * PS');xlabel('Distance (km)');ylabel('Depth (km)');
%*************************************************************************
