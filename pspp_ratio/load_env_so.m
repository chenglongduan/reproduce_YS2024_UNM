function [Zenv_final,Renv_final,Tenv_final,RTenv_final,geom_tab_final] = ...
    load_env_so(su_file1, su_file2, su_file3, su_file4, component, yn_snr_qc, snr_threshold, offset_min, offset_max)
% load envelope (env) data
% select traces by SNR (s) and offset (o)
% selection criteria are all based on 'su_file1' and 'Zenv' data
...
[Tenv_raw,~,~] = ReadSu(su_file3);
Tenv_raw(:,[8354,8011,8006,8181,8178]) = [];
Tenv = Tenv_raw(:,order_offset);
[RTenv_raw,~,~] = ReadSu(su_file4);
RTenv_raw(:,[8354,8011,8006,8181,8178]) = [];
RTenv = RTenv_raw(:,order_offset);
